...
def run_simulator(cfg):
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():            
            ...
            lidar_array = sm.get_lidar_obs()
            
            if (lidar_save_count<5):
                np.save(f"/home/jerry/Desktop/isaac-go2-ros2/lidar_{lidar_save_count}.npy", lidar_array)
                lidar_save_count += 1
            #print("depth", sm.get_depth_obs())
            
            depth_array = sm.get_depth_obs()
            # print("depth", depth_array)
            # np.save(f"/home/jerry/Desktop/isaac-go2-ros2/depth.npy", depth_array)
            # print(f"✅ Saved depth frame {depth_save_count}")

            #show_depth_image(depth_array, H=480, W=640, clip_max=5.0)
